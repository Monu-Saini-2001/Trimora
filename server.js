...
async function sendOtpEmail(toEmail, subject, otp) {
  ...
  try {
    ...
    const mailOptions = {
      from: `"LUZOX Support" <${process.env.EMAIL_USER}>`,
      to: toEmail,
      subject: subject,
      text: `Your LUZOX verification OTP is: ${otp}. It is valid for 5 minutes.`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; border: 1px solid #ddd; border-radius: 8px;">
          <h2 style="color: #c99e32; text-align: center;">LUZOX Verification Code</h2>
          <p>Hello,</p>
          <p>Your one-time password (OTP) to complete your request on LUZOX is:</p>
          ...
          <p style="font-size: 11px; color: #888; text-align: center;">This is an automated email from LUZOX Salon App.</p>
        </div>
      `
    };
...
app.post('/api/owners/register',async(req,res)=>{try{const{salonId,salonName,mobile,email,password,ownerName,address,coords,image,type}=req.body;const exists=await Owner.findOne({$or:[{mobile},{email}]});if(exists)return res.status(400).json({error:'Owner already registered with this mobile or email'});const tempId='temp_'+Date.now();const otp=Math.floor(100000+Math.random()*900000).toString();const tempOwner=new TempOwner({tempId,salonId,salonName,email,mobile,password,ownerName,address,coords,image,otp,type:type||'salon'});await tempOwner.save();const sent=await sendOtpEmail(email,"LUZOX Owner Verification Code",otp);if(sent){res.json({success:true,tempId})}else{res.json({success:true,tempId,otp})}}catch(err){res.status(500).json({error:err.message})}});
